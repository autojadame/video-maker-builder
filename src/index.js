...
const { concatParts, concatAudiosList } = require('./ffmpeg');
const createRenderer = require('./renderer');
const detectGPUBrand = require('./detect_encoder')
...
function Editor({
  ...
  async function edit({
    ...
    const gpuBrand = await detectGPUBrand()
    console.log("GPU: "+gpuBrand)
    ...
    try {
      ...
      const renderers = parts.map((part, partNum) => {
        ...
        return renderPart({ partNum, gpuBrand,partStart, partEnd, onProgress });
      });
      ...
      if (audioSamples.length > 0) {
        await concatAudiosList({ ffmpegPath, videoPath: finalOutPath, audios: audioSamples, tempDir, rawOutput });
      }
    } finally {
